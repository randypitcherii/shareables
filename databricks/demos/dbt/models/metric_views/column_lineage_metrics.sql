...
# (metric_views/column_lineage.yaml). system.* references are rewritten to dbt
# source() calls for lineage; the metric-view YAML is otherwise unchanged.

version: 1.1

# Fact: system.access.column_lineage (one column read/write lineage event).
# Joins to query.history AS A DIMENSION via the documented statement_id FK. N:1.
source: {{ source('system_access', 'column_lineage') }}

comment: Unity Catalog column lineage events with workspace and originating-query context.
...
dimensions:
  ...
  - name: Source Table
    ...
    comment: Three-level name to identify the source table.
  - name: Source Column
    expr: source.source_column_name
    comment: The name of the source column.
  - name: Target Table
    expr: source.target_table_full_name
    comment: Three-level name to identify the target table.
  - name: Target Column
    expr: source.target_column_name
    comment: The name of the target column.
  - name: Created By
...
measures:
  - name: Lineage Events
    expr: COUNT(1)
    comment: Number of column lineage events in the slice.
    format:
      type: number
      decimal_places:
        type: exact
        places: 0
  - name: Distinct Source Columns
    expr: COUNT(DISTINCT source.source_table_full_name || '.' || source.source_column_name)
    comment: Distinct number of source columns in the slice.
    format:
      type: number
      decimal_places:
        type: exact
        places: 0
  - name: Distinct Target Columns
    expr: COUNT(DISTINCT source.target_table_full_name || '.' || source.target_column_name)
    comment: Distinct number of target columns in the slice.
    format:
      type: number
      decimal_places:
        type: exact
