...
_git_info_cache: tuple[str | None, str | None] | None = None
...
def get_git_info() -> tuple[str | None, str | None]:
    ...
    for cwd in [Path(__file__).parent.parent, Path.cwd()]:
        try:
...
def version_string() -> str:
    """Return formatted version: v{semver} · {hash} · {date}"""
    git_hash, git_date = get_git_info()
    parts = [f"v{SEMVER}"]
    if git_hash:
        parts.append(git_hash)
    if git_date:
        parts.append(git_date)
    return " · ".join(parts)
