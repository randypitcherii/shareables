"""Bash-over-REST shell endpoints.

Ported from the randy_apps_starter template. Provides:
- /api/v1/shell/run     - execute a command and return stdout/stderr
- /api/v1/shell/stream  - execute a command with streaming output
- /api/v1/shell/complete - tab-completion for paths and commands

CWD is tracked per session using a UUID marker pattern that gets
injected into bash commands and stripped from output.
"""
...
logger = logging.getLogger("better-agent-gateway.shell")

router = APIRouter()

# ---------------------------------------------------------------------------
# Session CWD persistence (SQLite)
# ---------------------------------------------------------------------------

SESSION_ID_RE = re.compile(r"^[A-Za-z0-9._-]{1,128}$")
SESSION_STATE_DB_PATH = os.getenv(
    "SESSION_STATE_DB_PATH",
    "/tmp/better_agent_gateway_session_state.sqlite3",
)
