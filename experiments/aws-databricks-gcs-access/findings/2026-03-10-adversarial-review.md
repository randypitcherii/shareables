...
| 7 | No write-path testing (only reads) | **Medium** | Incomplete scope | Users may need bidirectional access |
...
This is a gap because the user likely needs a production job, not an interactive notebook.
...
The experiment only tested *reads*. If the user needs to write back to GCS (e.g., processed results, exports), the approaches need re-evaluation. The Python SDK can write, but `google-cloud-storage` uploads are single-threaded and not suitable for large writes without additional tooling.
