...
```
Could not resolve hostname error for HTTP GET to '/api/2.1/unity-catalog/schemas?catalog_name=<your-catalog>'
```
...
ATTACH '<your-catalog>' AS uc (TYPE UC_CATALOG);
```
...
ATTACH '<your-catalog>' AS uc (TYPE UC_CATALOG, SECRET my_uc_secret);
```
